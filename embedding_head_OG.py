...
class EmbeddingHead(torch.nn.Module):
    def __init__(self, hidden_units=HIDDEN_UNITS, emb_out=EMB_OUT, dropout_rate=DROPOUT_RATE):
        super(EmbeddingHead, self).__init__()
        self.linear1 = torch.nn.Linear(EMB_IN, int(hidden_units))
        self.linear2 = torch.nn.Linear(int(hidden_units), int(emb_out))
...
def train(model, train_dataloader, validation_dataloader, num_epochs, save_weights=True, lr=LR, pe_weight=PE_WEIGHT,
    ...
    for epoch in range(num_epochs):
        model.train(True)
        parameters.set_sample_size(30 * math.ceil(epoch / 6) + 15)
        train_loss = 0.0
        i = 0
        ...
        val_precision = v_losses[-3]
        val_recall = v_losses[-2]
        val_f1 = v_losses[-1]
        if epoch > 23:
        ...
        if verbose:
            pbar.set_description(
                f'Epoch {epoch + 1}/{num_epochs} - loss: {train_loss:.3f} - val_loss: {val_loss:.3f} - val_f1: {val_f1:.3f} - lr: {lr_scheduler._last_lr[-1]:.6f} - SS: {get_sample_size()}')
            pbar.update()

        if save_weights:
            if epoch > 23:
                if epoch == 23 or current_metric > max(history[metric][:-1]):
...
def evaluate_model(model, validation_dataloader, pe_weight=None, dynamic_cutoff=False,
    ...
    if PREPROCESSING_DATASET_TYPE == 'test':
        pbar = tqdm(total=len(q_dataloader), desc='Evaluating')

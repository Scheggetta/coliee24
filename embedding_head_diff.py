...
class EmbeddingHead(torch.nn.Module):
    def __init__(self):
        ...
        self.linear1 = torch.nn.Linear(EMB_IN, HIDDEN_SPACE_SIZE)
        self.linear2 = torch.nn.Linear(HIDDEN_SPACE_SIZE, 1)
    ...
    def forward(self, query):
        ...
        query = self.sig(self.linear2(query))
...
def train(model, train_dataloader, validation_dataloader, num_epochs, save_weights=True):
    ...
    for epoch in range(num_epochs):
        ...
        for dl_row in train_dataloader:
            ...
            query_out = torch.empty((0,)).to('cuda')
            GT = torch.empty((0,)).to('cuda')
            query_out.requires_grad = True
            GT.requires_grad = True

            for index in range(len(pe)):
                for el in [query[index] - e for e in pe[index].to('cuda')]:
                    query_out = torch.cat((query_out, model(el)))
                    GT = torch.cat((GT, torch.Tensor([1.0]).to('cuda')))
                for el in [query[index] - e for e in ne[index].to('cuda')]:
                    query_out = torch.cat((query_out, model(el)))
                    GT = torch.cat((GT, torch.Tensor([0.0]).to('cuda')))

            loss = loss_function(query_out, GT)
            loss.backward()
    ...
    with torch.no_grad():
        for q_name, q_emb in q_dataloader:
            ...
            for d_name, d_emb in d_dataloader:
                ...
                val_loss += loss_function(query_out, GT)
                pe_val_loss += 0.0 if len(pe) == 0 else loss_function(query_out[GT == 1.0], GT[GT == 1.0])
                ne_val_loss += loss_function(query_out[GT == 0.0], GT[GT == 0.0])
